...
async function main() {
  try {
    ...
    if (!existingAdmin) {
      ...
      logger.info(ℹ Admin user already exists: ${adminEmail});
    }
    ...
    const sampleCoaches = [
      {
        ...
        coach: {
          ...
          experienceDescription:
            "I have been teaching mathematics for over 8 years, specializing in algebra, calculus, and geometry. I hold a Master's degree in Mathematics Education and have helped hundreds of students improve their math skills.",
      ...
      },
      {
        ...
        coach: {
          ...
          experienceDescription:
            "Software engineer turned educator with 10+ years in the tech industry and 5 years teaching programming. I specialize in Python, JavaScript, and web development, making complex concepts accessible to beginners.",
      ...
      },
      {
        ...
        coach: {
          ...
          experienceDescription:
            "English Literature professor with a PhD in Victorian Literature. I have been teaching for 12 years and specialize in creative writing, literary analysis, and essay composition. I believe in making literature accessible and engaging for all students.",
      ...
      },
      {
        ...
        coach: {
          domain: "Physics",
          experienceDescription:
            "Physics researcher and educator with 15 years of experience. I hold a PhD in Theoretical Physics and have published numerous papers. I enjoy breaking down complex physics concepts into understandable lessons.",
          ...
          status: "REJECTED",
          rejectionReason:
            "Insufficient teaching experience documentation provided",
        },
      },
    ];
    ...
    for (const coachData of sampleCoaches) {
      ...
      if (!existingUser) {
        ...
        logger.info(
          ✅ Sample coach created: ${result.user.email} (${result.coach.status})
        );
      }
    }
        ...
        materials: [
          {
            ...
            url: "https://example.com/js-advanced.pdf",
          },
        ],
      },
    ];
    ...
    for (const coach of approvedCoaches) {
      if (coach.domain === "Computer Science") {
        for (const courseData of sampleCourses) {
          ...
          if (!existingCourse) {
            // Remove 'materials' property before creating the course
            const { materials, ...courseDataWithoutMaterials } = courseData;
            const course = await prisma.course.create({
              data: {
                ...courseDataWithoutMaterials,
                coachId: coach.userId,
              },
            });

            logger.info(
              ✅ Sample course created: ${course.title} by ${coach.user.firstName} ${coach.user.lastName}
            );
          }
        }
      }
    ...
    logger.info(
      "👨‍🏫 Sample Coaches: coach1@example.com (PENDING), coach2@example.com (APPROVED), coach3@example.com (PENDING), coach4@example.com (REJECTED)"
    );
    ...
    const sampleCoach = await prisma.user.findFirst({
      ...
      if (!sessionExists) {
        ...
        await prisma.session.create({
          data: {
            ...
            description:
              "This is a sample session for testing payments and booking.",
            startTime: now,
            endTime: end,
            duration: 60,
            status: "SCHEDULED",
        ...
        logger.info("ℹ Sample session already exists.");
      }
    } else {
      logger.warn(
        "⚠ Could not create sample session: missing course, coach, or parent."
      );
    }
