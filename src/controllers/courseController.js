...
export const getCourseById = asyncHandler(async (req, res) => {
  ...
  // 1. Get the course and its coach (User) info
  ...
  // 2. Get the Coach record for this user (if you need Coach-specific fields)
  let coachDetails = null;
  if (course && course.coach && course.coach.id) {
    coachDetails = await prisma.coach.findUnique({
      where: { userId: course.coach.id },
      select: {
        id: true,
        rating: true,
        totalReviews: true,
  ...
  const formattedCourse = {
    ...
    coach: {
      ...
      totalReviews: coachDetails?.totalReviews ?? 0,
      totalStudents: coachDetails?.totalStudents ?? 0,
      experience: coachDetails?.experienceDescription ?? null,
      languages: coachDetails?.languages ?? [],
    },
